...
def num_teachers_and_mobiles
  ...
  execute(<<-SQL)
    SELECT
      COUNT(teachers.name) AS count_teachers, COUNT(teachers.mobile) AS count_mobiles
    FROM
      teachers
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT
      depts.name, COUNT(teachers.id) AS num_of_staff
    FROM
      teachers
    RIGHT JOIN
 ...
 # SELECT depts.name, COUNT(depts.name) FROM teachers JOIN depts ON teachers.dept_id = depts.id GROUP BY depts.name;
